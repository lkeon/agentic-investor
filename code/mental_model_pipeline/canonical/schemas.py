"""Define Pydantic schemas and enums for canonical models, constitution placement, and graph edges."""
...
from mental_model_pipeline.fragments.schemas import DecisionStage, FragmentKind
...
class ConstitutionDomain(str, Enum):
    ...
    VALUATION_AND_EXPECTED_RETURN = "valuation_and_expected_return"
...
class CanonicalModelDraft(BaseModel):
    ...
    mechanism: list[str] = Field(default_factory=list, max_length=4)
    conditions: list[str] = Field(default_factory=list, max_length=4)
    failure_conditions: list[str] = Field(default_factory=list, max_length=4)
    decision_implications: list[str] = Field(default_factory=list, max_length=4)
    decision_stages: list[DecisionStage] = Field(default_factory=list)
    contextual_regimes: list[str] = Field(default_factory=list, max_length=4)
    supporting_fragment_codes: list[str] = Field(min_length=1)
    ...
    @classmethod
    def clean_fragment_codes(cls, values: list[str]) -> list[str]:
        cleaned: list[str] = []
        seen: set[str] = set()
...
class CanonicalClusterResult(BaseModel):
    ...
    models: list[CanonicalModelDraft] = Field(min_length=1)


class CanonicalisationBatchResult(BaseModel):
    model_config = ConfigDict(extra="forbid")

    clusters: list[CanonicalClusterResult] = Field(default_factory=list)
...
class ConstitutionAssignment(BaseModel):
    ...
    concept_family: str = Field(min_length=1, max_length=100)

    @model_validator(mode="after")
    def remove_primary_and_duplicates(self) -> ConstitutionAssignment:
        cleaned: list[ConstitutionDomain] = []

        for domain in self.secondary_domains:
            if domain != self.primary_domain and domain not in cleaned:
...
class ConstitutionBatchResult(BaseModel):
    ...
    assignments: list[ConstitutionAssignment] = Field(default_factory=list)
...
class RelationshipDraft(BaseModel):
    ...
    @model_validator(mode="after")
    def prevent_self_edge(self) -> RelationshipDraft:
        if self.source_canonical_code == self.target_canonical_code:
            raise ValueError("A model cannot connect to itself.")
        return self
...
class HierarchyRelationshipBatchResult(BaseModel):
    ...
    pairs: list[PairRelationshipResult] = Field(default_factory=list)


class CanonicalModelData(BaseModel):
    """Application representation used before database insertion."""

    model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)

    canonical_code: str = Field(pattern=r"^mmc_[a-z0-9]+_[a-f0-9]{8}$")
    ...
    decision_stages: list[DecisionStage] = Field(default_factory=list)
    contextual_regimes: list[str] = Field(default_factory=list)
    supporting_fragment_codes: list[str] = Field(min_length=1)
    evidence_confidence: float = Field(ge=0.0, le=1.0)
    investor_importance: float = Field(ge=0.0, le=1.0)
